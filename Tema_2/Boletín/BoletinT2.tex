...
\curso{2023/2024} % Do not show year
...
\begin{itemize}
\item Modulaciones analógicas lineales: AM y DBL
\item Manejo de fasores. Cálculo de componentes en fase y cuadratura de una señal.
\item Detectores: detector síncrono y de envolvente
\item Modulaciones angulares. Ancho de banda de transmisión e interferencia. Preénfasis y deénfasis.
\item Receptor superheterodino
\end{itemize}
}
{
	...
	\begin{itemize}
		\item Linear analog modulations: AM and DSB.
		\item Phasors. In-phase and quadrature components of a signal
  		\item Detectors: synchronous and envelope detectors.
  		\item Angular modulations. Bandwidth and interference. Preemphasis and deemphasis.
  		\item Superheterodine receiver. 
	\end{itemize}
}
...
\Problema{
	\cite{Haykin} La señal AM $s(t) = A_p (1 + m x(t)) \cos(2\pi f_p t)$ se pasa a través del sistema de la figura.

	\begin{center}
		\includegraphics[width=10cm]{Figuras/ProblemaHaykin}
	\end{center}

	Suponiendo que el mensaje está normalizado y su ancho de banda es $W$, que $0\leq m \leq 1$ y que $f_p > 2W$, demostrar que es posible recuperar el mensaje a partir de la salida del sistema.
}{
	Es posible. Basta con que la frecuencia de corte del filtro paso bajo sea $2W$
}{
	The AM signal $s(t) = A_p (1 + m x(t)) \cos(2\pi f_p t)$ is applied to the system shown in the figure.

	\begin{center}
		\includegraphics[width=10cm]{Figuras/ProblemaHaykin_en}
	\end{center}

	Assuming that the message is normalized, its bandwidth is $W$, that $0\leq m \leq 1$ and that $f_p > 2W$, show that the message can be obtained from the system output.
}{
	It's possible just selecting the cut-off frequency of the low-pass filter to be $2W$
}

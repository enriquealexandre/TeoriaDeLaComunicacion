...
\Problema{
	...
	\cite{Carlson} An FM signal plus noise has a received power of $1 \textrm{nW}$, $W_x = 500 \textrm{kHz}$, $S_{xn} = 0.1$, $|x(t)|_{max} = 4\textrm{V}$, $\omega_d = 500 \textrm{kHz/V}$ and $N_0 = 4\cdot 10^{-20} \textrm{W/Hz}$. Find $(S/N)_D$ in dB for FM detection and for deemphasized FM detection with $B_{de} = 5 \textrm{kHz}$.
}{
	\begin{itemize}
		\item FM: $(S/N)_D = 53.8dB$

...
\Problema{
	\cite{Carlson} Una señal FM tiene una potencia en recepción de $1 \textrm{nW}$, $W_x = 500 \textrm{kHz}$, $S_x = 0.1$, $|x(t)|_{max} = 4\textrm{V}$, $\omega_d = 500 \textrm{kHz/V}$ y $N_0 = 4\cdot 10^{-20}$. Calcula la relación señal a ruido en postdetección de dB para el caso sin filtro de preénfasis y para el caso de utilizar un filtro de preénfasis con $B_{de} = 5 \textrm{kHz}$.

}{
	\begin{itemize}
		\item FM: $(S/N)_D = 53.8dB$
  		\item FM con preénfasis: $(S/N)_D = 89dB$
	\end{itemize}
}{
	\cite{Carlson} An FM signal plus noise has a received power of $1 \textrm{nW}$, $S_x = 0.1$, $|x(t)|_{max} = 4\textrm{V}$, $\omega_d = 500 \textrm{kHz/V}$ and $N_0 = 4\cdot 10^{-20}$. Find $(S/N)_D$ in dB for FM detection and for deemphasized FM detection with $B_{de} = 5 \textrm{kHz}$.
}{
	\begin{itemize}
		\item FM: $(S/N)_D = 53.8dB$
  		\item deemphasized FM: $(S/N)_D = 89dB$
	\end{itemize}
}
